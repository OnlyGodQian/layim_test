...
var config = {
    ...
    right: -232,
    api: {
        friend: 'http://localhost:8080/rent/api/v1/user', //好友列表接口
        group: 'static/json/group.json', //群组列表接口 
        chatlog: 'static/json/chatlog.json', //聊天记录接口
        groups: 'static/json/groups.json', //群组成员接口
        sendurl: 'http://localhost:8080/rent/api/v1/task/data' //发送消息接口
    },
    user: { //当前用户信息
        name: 'http://localhost:8080/rent/api/v1/username',
        face: 'http://localhost:8080/rent/api/v1/userimg'
    },
...
xxim.transmit = function(){
    ...
    log.send = function(){
        ...
        if(data.content.replace(/\s/g, '') === ''){
            ...
            //send message to server with sendurl defined before
            /*that.json(config.api.sendurl, data, function(datas){
            	
            });*/
            //TODO sendtoserver(config.api.sendurl, data);
            var transdata = 'id='+data.id+'&content='
            	+data.content+'&sign_key='
            	+data.sign_key+'&time='+data._;
            $.ajax({
				type: 'post',
